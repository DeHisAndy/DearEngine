...
#include "MathType.h"
#include <xmmintrin.h>
#include "..\..\ThirdParty\SSE\MathSSE.h"
#include "Vector.h"
#include "Quat.h"
#include "Rotator.h"
#include "Matrix.h"
...
#include "..\FunctionLibrary\KismetSystemLibrary.h"
#include "Vector4D.h"
...
FORCEINLINE VectorRegister Lerp(const VectorRegister& A, const VectorRegister& B, const VectorRegister& Alpha)
{
...
public:
	...
	*/
	...
	FORCEINLINE void Blend(const FTransform& Atom1, const FTransform& Atom2, float Alpha)
	{
		...
		// checkSlow that all bone atoms coming from animation are normalized
		checkSlow(Atom1.IsRotationNormalized());
		checkSlow(Atom2.IsRotationNormalized());
		...
		else
		{
			...
			Translation = Lerp(Atom1.Translation, Atom2.Translation, BlendWeight.Value);
			Scale3D = Lerp(Atom1.Scale3D, Atom2.Scale3D, BlendWeight.Value);
	...
	FORCEINLINE void BlendWith(const FTransform& OtherAtom, float Alpha)
	{
		...
		checkSlow(OtherAtom.IsRotationNormalized());
		...
		if (Alpha > ZERO_ANIMWEIGHT_THRESH)
		{
			...
			else
			{
				// Simple linear interpolation for translation and scale.				
				ScalarRegister BlendWeight = ScalarRegister(Alpha);
				Translation = Lerp(Translation, OtherAtom.Translation, BlendWeight.Value);

				Scale3D = Lerp(Scale3D, OtherAtom.Scale3D, BlendWeight.Value);
	...
	FORCEINLINE FVector4D	TransformFVector4(const FVector4D& V) const;
	FORCEINLINE FVector4D	TransformFVector4NoScale(const FVector4D& V) const;
	...
	*/
	...
	/**
	 * checkSlows the components for NaN's
...
public:
	...
	*/
	 ...
	 * Scale3D = Lerp(SourceAtom1.Scale3D, SourceAtom2.Scale3D, Alpha)
	 *
	...
	{
		Translation = Lerp(SourceAtom1.Translation, SourceAtom2.Translation, Alpha.Value);
		Scale3D = Lerp(SourceAtom1.Scale3D, SourceAtom2.Scale3D, Alpha.Value);
	...
	/**
	 * checkSlows whether the rotation component is normalized or not
	 *
	...
	FORCEINLINE static void BlendFromIdentityAndAccumulate(FTransform& FinalAtom, FTransform& SourceAtom, const ScalarRegister& BlendWeight)
	{
		...
		const VectorRegister BlendedTranslation = Lerp(VectorZero(), SourceAtom.Translation, BlendWeight.Value);
		const VectorRegister BlendedScale3D = Lerp(VectorZero(), SourceAtom.Scale3D, BlendWeight.Value);
...
private:
	...
	* @param	DesiredScale - there is no checkSlow on if the magnitude is correct here. It assumes that is correct.
...
FORCEINLINE FVector4D FTransform::TransformFVector4NoScale(const FVector4D& V) const
{
	DiagnosticCheckNaN_All();
...
FORCEINLINE FVector4D FTransform::TransformFVector4(const FVector4D& V) const
{
	DiagnosticCheckNaN_All();
