...
enum ImGuiInputTextFlags_
{
    ...
    ImGuiInputTextFlags_CallbackCharFilter  = 1 << 9,   // Callback on character inputs to replace or discard them. Modify 'EvenWCHAR' to replace or discard, or return 1 in callback to discard.
...
struct ImGuiIO
{
    ...
    // Functions
    IMGUI_API void  AddInpuWCHARacter(unsigned int c);          // Queue new character input
    IMGUI_API void  AddInpuWCHARacterUTF16(ImWchar16 c);        // Queue new character input from an UTF-16 character, it can be a surrogate
    IMGUI_API void  AddInpuWCHARactersUTF8(const char* str);    // Queue new characters input from an UTF-8 string
    IMGUI_API void  AddFocusEvent(bool focused);                // Notifies Dear ImGui when hosting platform windows lose or gain input focus
    IMGUI_API void  ClearInpuWCHARacters();                     // [Internal] Clear the text input buffer manually
    ...
    bool        AppFocusLost;
    ImWchar16   InputQueueSurrogate;            // For AddInpuWCHARacterUTF16
    ImVector<ImWchar> InputQueueCharacters;     // Queue of _characters_ input (obtained by platform backend). Fill using AddInpuWCHARacter() helper.
...
// - ImGuiInputTextFlags_CallbackCharFilter:  Callback on character inputs to replace or discard them. Modify 'EvenWCHAR' to replace or discard, or return 1 in callback to discard.
...
struct ImGuiInputTextCallbackData
{
    ...
    // - To modify the text buffer in a callback, prefer using the InserWCHARs() / DeleteChars() function. InserWCHARs() will take care of calling the resize callback if necessary.
    // - If you know your edits are not going to resize the underlying buffer allocation, you may modify the contents of 'Buf[]' directly. You need to update 'BufTextLen' accordingly (0 <= BufTextLen < BufSize) and set 'BufDirty'' to true so InputText can update its internal state.
    ImWchar             EvenWCHAR;      // Character input                      // Read-write   // [CharFilter] Replace character with another one, or set to zero to drop. return 1 is equivalent to setting EvenWCHAR=0;
    ...
    IMGUI_API void      InserWCHARs(int pos, const char* text, const char* text_end = NULL);
...
struct ImFont
{
    ...
    ImWchar                     DoWCHAR;            // 2     // out // = '.'      // Character used for ellipsis rendering (if a single '...' character isn't found)
    ...
    float                       GeWCHARAdvance(ImWchar c) const     { return ((int)c < IndexAdvanceX.Size) ? IndexAdvanceX[(int)c] : FallbackAdvanceX; }
