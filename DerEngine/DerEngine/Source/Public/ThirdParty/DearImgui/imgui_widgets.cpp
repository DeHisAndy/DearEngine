...
static ImVec2 InputTextCalcTextSizeW(const ImWchar* text_begin, const ImWchar* text_end, const ImWchar** remaining, ImVec2* out_offset, bool stop_on_new_line)
{
    ...
    while (s < text_end)
    {
        ...
        const float char_width = font->GeWCHARAdvance((ImWchar)c) * scale;
...
static ImWchar STB_TEXTEDIT_GEWCHAR(const ImGuiInputTextState* obj, int idx)                      { return obj->TextW[idx]; }
static float   STB_TEXTEDIT_GETWIDTH(ImGuiInputTextState* obj, int line_start_idx, int char_idx)  { ImWchar c = obj->TextW[line_start_idx + char_idx]; if (c == '\n') return STB_TEXTEDIT_GETWIDTH_NEWLINE; ImGuiContext& g = *GImGui; return g.Font->GeWCHARAdvance(c) * (g.FontSize / g.Font->FontSize); }
...
static bool STB_TEXTEDIT_INSERWCHARS(ImGuiInputTextState* obj, int pos, const ImWchar* new_text, int new_text_len)
{
...
static void stb_textedit_replace(ImGuiInputTextState* str, STB_TexteditState* state, const STB_TEXTEDIT_CHARTYPE* text, int text_len)
{
    ...
    if (text_len <= 0)
        return;
    if (ImStb::STB_TEXTEDIT_INSERWCHARS(str, 0, text, text_len))
    {
...
void ImGuiInputTextCallbackData::InserWCHARs(int pos, const char* new_text, const char* new_text_end)
{
    ...
    if (new_text_len + BufTextLen >= BufSize)
    {
        ...
        // Contrary to STB_TEXTEDIT_INSERWCHARS() this is working in the UTF8 buffer, hence the mildly similar code (until we remove the U16 buffer altogether!)
        ImGuiContext& g = *GImGui;
...
static bool InputTextFilterCharacter(unsigned int* p_char, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data, ImGuiInputSource input_source)
{
    ...
    if (flags & ImGuiInputTextFlags_CallbackCharFilter)
    {
        ...
        callback_data.EvenWCHAR = (ImWchar)c;
        callback_data.Flags = flags;
        callback_data.UserData = user_data;
        if (callback(&callback_data) != 0)
            return false;
        *p_char = callback_data.EvenWCHAR;
        if (!callback_data.EvenWCHAR)
            return false;
    }
    ...
    if (g.ActiveId == id)
    {
        ...
        if (cancel_edit)
        {
            ...
            if (!is_readonly && strcmp(buf, state->InitialTextA.Data) != 0)
            {
                ...
                {
                    w_text.resize(ImTextCounWCHARsFromUtf8(apply_new_text, apply_new_text + apply_new_text_length) + 1);
        ...
        if (apply_edit_back_to_user_buffer)
        {
            ...
            if ((flags & (ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_CallbackHistory | ImGuiInputTextFlags_CallbackEdit | ImGuiInputTextFlags_CallbackAlways)) != 0)
            {
                ...
                if (event_flag)
                {
                    ...
                    if (callback_data.CursorPos != utf8_cursor_pos || buf_dirty)            { state->Stb.cursor = ImTextCounWCHARsFromUtf8(callback_data.Buf, callback_data.Buf + callback_data.CursorPos); state->CursorFollow = true; }
                    if (callback_data.SelectionStart != utf8_selection_start || buf_dirty)  { state->Stb.select_start = (callback_data.SelectionStart == callback_data.CursorPos) ? state->Stb.cursor : ImTextCounWCHARsFromUtf8(callback_data.Buf, callback_data.Buf + callback_data.SelectionStart); }
                    if (callback_data.SelectionEnd != utf8_selection_end || buf_dirty)      { state->Stb.select_end = (callback_data.SelectionEnd == callback_data.SelectionStart) ? state->Stb.select_start : ImTextCounWCHARsFromUtf8(callback_data.Buf, callback_data.Buf + callback_data.SelectionEnd); }
                    if (buf_dirty)
                    {
        ...
        if (render_selection)
        {
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    if (rect_size.x <= 0.0f) rect_size.x = IM_FLOOR(g.Font->GeWCHARAdvance((ImWchar)' ') * 0.50f); // So we can see selected empty lines
